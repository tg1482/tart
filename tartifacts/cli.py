...
from . import discover, index, manifest as manifest_mod, refresh, roots as roots_mod, trust
from .manifest import Manifest

USAGE = (
    "usage: tart list | run <name> | render <name> [--json] | "
    "fetch <name> | register <path> | trust <name> | roots [add|rm <path>] | reindex | --skill"
)
...
def _fetch(ptr: Manifest) -> int:
    ...
    try:
        proc = subprocess.run(
            ptr.fetch, shell=True, cwd=ptr.root, env=env, timeout=refresh.FETCH_TIMEOUT
        )
    ...
    if ptr.data:
        ...
        if after is None:
            print(
                f"fetch exited 0 but {ptr.data_path} still does not exist.\n"
        ...
        if before is not None and after <= before:
            print(
                f"fetch exited 0 but did not rewrite {ptr.data_path}.\n"
                f"  it likely writes elsewhere — it should write to\n"
                f"  tartifacts.data_path(), which resolves to the manifest's \"data\".",
                file=sys.stderr,
            )
            return 1
